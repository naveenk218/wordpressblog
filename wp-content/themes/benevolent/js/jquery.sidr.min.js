...
if(displace){$body.addClass('sidr-animating').css({width:$body.width(),position:'absolute'}).animate(bodyAnimation,{queue:!1,duration:speed,complete:function(){$(this).addClass(bodyClass)}})}else{setTimeout(function(){$body.addClass(bodyClass)},speed)}
$menu.css('display','block').animate(menuAnimation,{queue:!1,duration:speed,complete:function(){sidrMoving=!1;sidrOpened=name;if(typeof callback==='function'){callback(name)}
...
if($body.is('body')){scrollTop=$html.scrollTop();$html.css('overflow-x','').scrollTop(scrollTop)}
$body.addClass('sidr-animating').animate(bodyAnimation,{queue:!1,duration:speed}).removeClass(bodyClass);$menu.animate(menuAnimation,{queue:!1,duration:speed,complete:function(){$menu.removeAttr('style').hide();$body.css({width:'',position:'',right:'',left:''});$('html').css('overflow-x','');sidrMoving=!1;sidrOpened=!1;if(typeof callback==='function'){callback(name)}
$body.removeClass('sidr-animating')}});onClose()}}};var publicMethodsIndex,publicMethods=['open','close','toggle'],methodName,methods={},getMethod=function(methodName){return function(name,callback){privateMethods.execute(methodName,name,callback)}};for(publicMethodsIndex=0;publicMethodsIndex<=publicMethods.length;publicMethodsIndex++){methodName=publicMethods[publicMethodsIndex];methods[methodName]=getMethod(methodName)}
$.sidr=function(method){if(methods[method]){return methods[method].apply(this,Array.prototype.slice.call(arguments,1))}else if(typeof method==='function'||typeof method==='string'||!method){return methods.toggle.apply(this,arguments)}else{$.error('Method '+method+' does not exist on jQuery.sidr')}};$.fn.sidr=function(options){var settings=$.extend({name:'sidr',speed:300,side:'left',source:null,renaming:!0,body:'body',displace:!0,onOpen:function(){},onClose:function(){}},options);var name=settings.name,$sideMenu=$('#'+name);if($sideMenu.length===0){$sideMenu=$('<div />').attr('id',name).appendTo($('body'))}
...
return this.each(function(){var $this=$(this),data=$this.data('sidr'),flag=!1;if(!data){sidrMoving=!1;sidrOpened=!1;$this.data('sidr',name);$this.bind('touchstart click',function(event){event.preventDefault();if(!flag){flag=!0;methods.toggle(name);setTimeout(function(){flag=!1},100)}})}})}})(jQuery)