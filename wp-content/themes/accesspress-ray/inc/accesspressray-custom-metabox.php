...
<tr>
<td colspan="4"><em class="f13"><?php esc_html_e('Choose Sidebar Template', 'accesspress-ray'); ?></em></td>
</tr>
...
<?php  
   foreach ($accesspress_ray_sidebar_layout as $field) {  
                ...
                <input type="radio" name="accesspress_ray_sidebar_layout" value="<?php echo esc_attr($field['value']); ?>" <?php checked( $field['value'], $accesspress_ray_sidebar_metalayout ); if(empty($accesspress_ray_sidebar_metalayout) && $field['value']=='right-sidebar'){ echo "checked='checked'";} ?>/>&nbsp;<?php echo esc_html($field['label']); ?>
                </label>
                </div>
                <?php } // end foreach 
                ?>
...
<tr>
    <td><em class="f13"><?php /* translators: %s : theme option page link */ echo sprintf(__('You can set up the sidebar content <a href="%s" target="_blank">here</a>', 'accesspress-ray'), esc_url(admin_url('/themes.php?page=theme_options'))); ?></em></td>
</tr>
</table>
...
function accesspress_ray_save_sidebar_layout( $post_id ) { 
    ...
    if ( !isset( $_POST[ 'accesspress_ray_sidebar_layout_nonce' ] ) || !wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST[ 'accesspress_ray_sidebar_layout_nonce' ] ) ), basename( __FILE__ ) ) )
        return;
    ...
    foreach ($accesspress_ray_sidebar_layout as $field) {  
        ...
        $new = isset( $_POST['accesspress_ray_sidebar_layout'] ) ? sanitize_text_field( wp_unslash( $_POST['accesspress_ray_sidebar_layout'] ) ) : '';
        if ($new && $new != $old) {  
